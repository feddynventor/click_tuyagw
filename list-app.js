...
async function listApp(config, options) {
	...
	console.log('>> Tool per hijack dispositivi Tuya WB2L per Click Tecnologie'.gray);
	console.log('Troubleshooting: https://github.com/codetheweb/tuyapi/issues/280\n\n'.gray);
	...
	console.log('URL Certificato: http://' + ips[0] + ':' + options.certPort + '/ca.pem')

	console.log('\n');

	// Instructions for setting up proxy
	console.log('PROXY: '.bold.blue + ips[0].bold.yellow + ' PORT: '.bold.blue + (String(options.port)).bold.yellow + '.'.bold.blue);
	if (ips.length > 1) {
